...
final class MelangeFaceDetector {
    ...
    private func detectWithMelangeIfAvailable(
        ...
        #if canImport(ZeticMLange)
        Logger.app.info(
            "Melange SDK is linked, but the face detection helper source is not present in this target. Add ZETIC's generated FaceDetectionWrapper/FaceDetectionResult source, then re-enable the Melange path."
        )
        return nil
        #else
